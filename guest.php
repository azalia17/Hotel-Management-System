...
$booking = "select bd.ssn, concat(bd.fname, ' ', bd.mnit, ' ', bd.lname) as gname, r.room_number, bd.contact_number, bd.email_address, bd.booking_date, bd.check_in_date, bd.check_out_date, bd.total_amount, concat(e.fname, ' ', e.mnit, ' ', e.lname) as employee from booking_details bd inner join employee e on (bd.emp_id = e.ssn) 
inner join rooms_booked rb on (bd.booking_id = rb.booking_id) inner join rooms r on (rb.room_id = r.room_id) ORDER BY bd.check_out_date ASC;";
